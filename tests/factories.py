...
class TibiaHuntAnalyserFactory(factory.Factory):
    ...
    world = factory.LazyAttribute(lambda: random.choice(['Honbra', 'Vunira', 'Serdebra'])),
    loot = loot,
    waste = waste,
    start_date = factory.LazyAttribute(lambda: get_current_datetime_formatted()),
    end_date = factory.LazyAttribute(lambda: get_current_datetime_formatted()),
